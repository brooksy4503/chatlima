...
| Feature | Cost |
|---------|------|
| Web Search (agentic, default) | 5 credits × invocation count (billed after stream from tool steps, UI tool parts, `usage.raw.server_tool_use_details.web_search_requests`, or OpenRouter generation `usage_web` / `num_search_results` fallback) |
...
- **Chat message list scrolling** (implementation):
  - **`components/messages.tsx`**: Owns the scrollable message list (`h-full min-h-0 min-w-0 overflow-y-auto overflow-x-hidden`). During `streaming` or `submitted` status, passes a `scrollTrigger` (message count + latest message text length) to the scroll hook so in-place token updates scroll the view without DOM structure changes.
  ...
  - **`app/(app)/layout.tsx`**: Chat shell uses `overflow-hidden` (not `overflow-auto`) with `min-w-0` so wide markdown/KaTeX cannot create a page-level horizontal scrollbar under the composer toolbar.
  - **Markdown / KaTeX overflow**: `components/markdown.tsx` wraps content in `min-w-0 w-full overflow-x-auto`. KaTeX styles in `app/globals.css` bound math to the message column (`max-width: 100%`); wide equations scroll inside the message, not the app shell.
- **Dual-Path File Upload**: Up to 5 files per message, 30 MB max per file. Images (JPEG, PNG, WebP) sent as base64 for vision; documents (PDF, CSV, Excel) and text/code files uploaded to Vercel Blob and exposed to the AI via a `read_file` tool (content parsed on demand). Parser limits: Excel 1,000 rows/sheet, CSV 10,000 rows; global content cap 1,000,000 characters per file (`applyContentCap` in `lib/file-reader/index.ts`).
...
- **Conversation branching**: Same-chat immutable branches via `messages.parentMessageId` and `chats.activeLeafMessageId`. **Regenerate** creates a new assistant sibling under the same user message; **Edit & resubmit** creates a new user sibling and streams a new assistant response (non-text parts such as file attachments are preserved unless removed in the editor); hidden branches remain in the graph. A compact `‹ n / m ›` pager on both user (edited prompts) and assistant (regenerated replies) messages switches versions and persists selection via `PATCH /api/chats/[id]/active-leaf`. Selecting a user sibling resolves the active leaf to that branch's deepest descendant so the matching assistant reply stays visible. **Fork to new chat** copies only the visible path through the selected message via `POST /api/chats/[id]/fork` and inherits the source project link. Stream-generating branch operations route through `POST /api/chat` `operation` (`continue`, `regenerate`, `edit-resubmit`) and `lib/chat/resolveChatOperation.ts`; `continue` merges only a valid extension onto the server-owned active path (`lib/chat/mergeContinuePath.ts`). Leaf selection and fork use the dedicated REST routes above, not `POST /api/chat`. Persistence uses upsert-by-id (`ConversationPersistenceService`) rather than full chat replace so sibling branches are preserved; `getChatById` and `loadChatGraph` share the same canonical path resolution. Legacy linear chats are backfilled lazily on load and via `pnpm db:backfill-parents` (dev) or `pnpm db:backfill-parents:prod` after `vercel env pull .env.production.local --environment=production`. Client session lifecycle lives in `hooks/useChatSession.ts` (transport, DB sync via `lib/chat/adoptDbMessages.ts`, recovery); `components/chat.tsx` composes session, compare, and branch UI.
...
- **Display**: Compare assistants are graph siblings under the compare user message (same `parentMessageId` as branching). `buildActivePathMessages` returns only the active-leaf branch, so client hydration uses `buildChatDisplayMessages` / `useChatDisplayMessages` (`lib/chat/buildChatDisplayMessages.ts`, `hooks/useChatDisplayMessages.ts`) to expand each comparison turn before rendering the compare timeline or adopting DB state. Branch switches apply the same expansion via `buildCompareDisplayPath`. **Continue with this** (`POST /api/compare/promote`) clears `comparisonTurnId` on **all** messages in the comparison turn (user + every model response), sets `activeLeafMessageId` to the chosen assistant, and exits compare mode so the chat continues on a single model. Non-promoted responses remain as normal branch siblings (‹ n / m › pager) with edit/resubmit/fork. Client hydration also strips orphan assistant-only `comparisonTurnId` values left by older partial promotes.
```
